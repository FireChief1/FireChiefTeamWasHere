...
class StaticWebDeveloperAgent(DeveloperAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "one. Keep its exact subject, title, headings, text, and links. If "
            "the task is about design, style, layout, or 'making it modern / "
            "better looking', change ONLY the CSS, classes, and markup "
            "structure -- never the wording or the topic, and never invent new "
            "placeholder content (do not swap the subject for animals, "
            "products, lorem ipsum, etc.). Replace the subject matter only when "
            "the user explicitly asks for different content.\n\n"
